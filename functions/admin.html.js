...
export async function onRequest(context) {
  ...
  if (request.method === 'POST') {
    ...
    if (hash === ADMIN_HASH) {
      ...
      response.headers.set('Set-Cookie', `${COOKIE_NAME}=${ADMIN_HASH}; HttpOnly; Secure; SameSite=Strict; Path=/; Max-Age=86400`);
      return response;
    }
    return new Response(LOGIN_HTML.replace('{error}', 'Contrase\u00f1a incorrecta'), {
      status: 403,
